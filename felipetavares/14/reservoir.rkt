...
(define (p->p~ env p threshold)
  (let ([b (vec2-add p below)]
    ...
    (cond
      [(>= (vec2-y p) threshold) 'straight-through]
      [(free? env b) b]
      [(free? env bl) bl]
      [(free? env br) br]
      [else 'rest])))
...
(define (solution input)
  ...
  (define threshold (apply max (map vec2-y (flatten lines))))
  (define env (build-env lines))

  (let-values ([(_ number) (simulate-until-done env threshold)])
    number))

(check-equal? (solution "test/1") 24)
