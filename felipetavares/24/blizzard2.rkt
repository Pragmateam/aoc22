...
(define (shortest-time walls blizzards w h lm end pos (t 0) (cache (make-hash)))
  (define key (cons (modulo t (add1 lm)) pos))

  (if (or (equal? pos end) (>= t 350))
      t
      (if (hash-has-key? cache key)
          (+ t (hash-ref cache key))
...
(define (solution input)
  ...
  (define b0 (move-blizzards blizzards w h))
  (define t0 (shortest-time (list->set walls) b0 w h (lcm (- w 1) (- h 1)) end start))
  (define b1 (repeat t0 (lambda (b) (move-blizzards b w h)) b0))
  (define t1 (shortest-time (list->set walls) b1 w h (lcm (- w 1) (- h 1)) start end))
  (define b2 (repeat t1 (lambda (b) (move-blizzards b w h)) b1))
  (define t2 (shortest-time (list->set walls) b2 w h (lcm (- w 1) (- h 1)) end start))

  (+ t0 t1 t2))

(check-equal? (solution "test/1") 54)
