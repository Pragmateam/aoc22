  ...
  (define processed-hmap (for/list ([y (range h)])
                           (for/list ([x (range w)])
                             (match (list-ref (list-ref hmap y) x)
                               [#\S (set! start (point x y 0 '())) (char->integer #\a)]
                               [#\E (set! end (point x y 0 '())) (char->integer #\z)]
                               [letter (char->integer letter)]))))

  (values processed-hmap start end))

(define (solution input)
  (define-values (hmap start end) (parse input))
  (sub1 (length (a* hmap (set) (list start) end))))

(check-equal? (solution "test/1") 31)
